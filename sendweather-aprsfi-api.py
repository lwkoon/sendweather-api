...
try:
    ...
    bash_command = f"""
    /root/aprs-weather-submit/aprs-weather-submit --callsign YOURCALLSIGN-13 --latitude 10.755255 --longitude 115.402032 --altitude 10 --server asia.aprs2.net --port 14580 --username USERCALLSIGN --password YOURPASSWORD -M "Sky conditions: {weather_condition} CALLSIGN-API WX" -T "{aprs_temperature:.2f}" -h "{aprs_humidity}" -b "{aprs_pressure:.1f                                                                                }" > /dev/null 2>&1
    """
